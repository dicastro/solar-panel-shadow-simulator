...
function PointWithShadow({ position, sunDir, isDaylight, raycaster }: any) {
  const meshRef = useRef<THREE.Mesh>(null!);
  const [isShaded, setIsShaded] = useState(false);
  const lastUpdate = useRef(0); // Para controlar el tiempo

  useFrame((state) => {
    if (!isDaylight || !meshRef.current) return;
    ...
    raycaster.set(worldPosition, sunDir);

    // 3. Detectar colisiones con objetos que proyectan sombra
    // Filtramos la escena para no colisionar con el propio panel (opcional pero recomendado)
    ...
    if (hasObstacle !== isShaded) setIsShaded(hasObstacle);
  });

  return (
    <mesh ref={meshRef} position={position}>
      <sphereGeometry args={[0.01, 8, 8]} />
      <meshBasicMaterial color={!isDaylight ? "#333" : (isShaded ? "#ff4444" : "#ccff00")} />
    </mesh>
  );
}
...
function SolarPanel({ 
  ...
  // Calculamos la dirección del sol para este instante exacto
  const sunPos = SunCalc.getPosition(date, config.geometry.latitude, config.geometry.longitude);
          ...
          <group key={i}>
            ...
            {showPoints && [...Array(density)].map((_, row) => 
              [...Array(density)].map((_, col) => {
                // calculate relative position in the zone
                //const pX = posX + (col - (density - 1) / 2) * (zWidth / (density + 1));
                //const pZ = posZ + (row - (density - 1) / 2) * (zHeight / (density + 1));
 ...
 */
function SolarArray({ array, arrayIndex, defaults, settings, centerX, centerZ, showPoints, density, date, config }: any) {
...
function Scene({ config, date, showPoints, density }: { config: Config; date: Date, showPoints: boolean, density: number }) {
  ...
  return (
    <div className="main-container" style={{ width: '100vw', height: '100vh', background: '#050505', position: 'relative' }}>
      <MainControls 
        ...
        adjustDate={adjustDate} config={config} 
      />
      ...
      <Canvas shadows camera={{ position: [0, ORBIT_MAX_DISTANCE, 70], fov: 40 }}>
        <Scene config={config} date={date.toDate()} showPoints={showPoints} density={density} />
      </Canvas>
    </div>
  )
}
